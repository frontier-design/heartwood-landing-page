...
import { overlayRenderers, OVERLAY_FADE_MS } from "./overlays.js";
...
export class DotFieldEngine {
  ...
  setDensityScale(scale) {
    ...
    if (this._scrubActive) this.seek(this._scrubProgress);
      ...
      this.setLayout(this._p, this._current.name, this._current.opts, {
        instant: true,
      });
    }
  }
  ...
  _build(name, opts, total, rand) {
    ...
    const result = build(total, this.w, this.h, {
      ...opts,
      rand,
      densityScale: this.densityScale,
    });
    if (Array.isArray(result)) return { positions: result, meta: null };
  ...
  _resolveState(index) {
    ...
    if (!state) return { positions: [], meta: null };
    const total = Math.max(
      0,
      Math.round((state.opts.count ?? this.count) * this.densityScale),
    );
    const resolved = this._build(
      state.name,
      state.opts,
      total,
      mulberry32(this._seedForState(index)),
    );
    this._stateCache[index] = resolved;
    return resolved;
  }

  _positionsFor(index) {
    return this._resolveState(index).positions;
  }

  _metaFor(index) {
    return this._resolveState(index).meta;
  }
  ...
  getSegmentAndPhase(progress) {
    ...
    if (n <= 1)
      return { segment: 0, phase: "hold", phaseT: 0, fromIndex: 0, toIndex: 0 };
    ...
    if (last.type === "hold")
      return {
        ...
        phase: "hold",
        phaseT: 1,
        fromIndex: last.kfIndex,
        toIndex: last.kfIndex,
      };
    return {
      segment: phases.length - 1,
      phase: "transition",
  ...
  seek(progress) {
    ...
    if (this.states.length <= 1) {
      ...
      this._scrubOverlay = {
        phase: "hold",
        phaseT: 0,
        from: this._metaFor(0),
        to: null,
      };
      return;
    }
    const { phase, phaseT, fromIndex, toIndex } = this.getSegmentAndPhase(
      this._scrubProgress,
    );
    ...
    this._scrubOverlay = {
      phase,
      phaseT,
      from: this._metaFor(fromIndex),
      to: phase === "hold" ? null : this._metaFor(toIndex),
    };
  }
  ...
  _snapToInterpolated(fromIndex, toIndex, t) {
    ...
    for (let i = 0; i < this.dots.length; i++) {
      ...
      if (i < minDots) {
        ...
        const aA = posA[i].alpha ?? 255,
          aB = posB[i].alpha ?? 255;
        d.alpha = aA + (aB - aA) * e;
        const dA = posA[i].diam ?? this.dotDiameter,
          dB = posB[i].diam ?? this.dotDiameter;
        d.diam = dA + (dB - dA) * e;
  ...
  setLayout(p, name, opts = {}, control = {}) {
    ...
    const { positions, meta } = this._build(
      name,
      opts,
      total,
    ...
    this._setOverlay(p, meta, transition);
  ...
  _setOverlay(p, meta, transition = {}) {
    ...
    if (transition.instant) {
      this._overlayPrev = null;
      this._overlay = meta ? { meta, t0: now - OVERLAY_FADE_MS } : null;
      return;
    }
    if (this._overlay?.meta)
      this._overlayPrev = { meta: this._overlay.meta, t0: now };
    if (!meta) {
      this._overlay = null;
      return;
    }
    const stagger = transition.stagger ?? MAX_STAGGER_MS;
    const dur = Math.max(1, transition.duration ?? SEEK_DURATION_MS);
    const delay = meta.textDelay ?? stagger + dur;
  ...
  drawOverlays(p, rgb) {
    ...
    const now = p.millis();
    if (this._overlayPrev) {
      const e = now - this._overlayPrev.t0;
      if (e >= OVERLAY_FADE_MS) this._overlayPrev = null;
      else
        render(
          this._overlayPrev.meta,
          1 - easeInOutCubic(Math.max(0, e) / OVERLAY_FADE_MS),
        );
    }
    if (this._overlay) {
      const e = now - this._overlay.t0;
      if (e > 0)
        render(
          this._overlay.meta,
          e >= OVERLAY_FADE_MS ? 1 : easeInOutCubic(e / OVERLAY_FADE_MS),
        );
    }
  }
  ...
  _transitionTo(p, positions, opts = {}) {
    ...
    for (let i = 0; i < this.dots.length; i++) {
      ...
      if (i >= used) {
        ...
        d.alphaTo = 0;
        d.alphaT0 = now;
        d.alphaDur = instant ? 1 : 500;
        d.diamFrom = d.diam;
        d.diamTo = 0;
        d.diamT0 = now;
        d.diamDur = instant ? 1 : 500;
        continue;
      }
      ...
      d.seekT0 = now + delay;
      d.seekDur = dur;
      d.seekAfter = seekAfter;

      d.diamFrom = d.diam;
      d.diamTo = pos.diam ?? stateDiam;
      d.diamT0 = now + delay;
      d.diamDur = dur;

      d.alphaFrom = d.alpha;
      d.alphaTo = pos.alpha ?? 255;
      d.alphaT0 = now + delay;
      d.alphaDur = dur;
    }
  }
  ...
  _applyCursor(p) {
    ...
    if (mx < -rad || mx > this.w + rad || my < -rad || my > this.h + rad) {
      ...
      for (const d of this.dots) {
        ...
        if (Math.abs(d.nudgeX) > 0.01 || Math.abs(d.nudgeY) > 0.01)
          moving = true;
        else {
          d.nudgeX = 0;
          d.nudgeY = 0;
    ...
    for (const d of this.dots) {
      if (d.mode === "wander") {
        ...
        if (d.x < this.dotR) {
          d.x = this.dotR;
          vx *= -1;
        } else if (d.x > this.w - this.dotR) {
          d.x = this.w - this.dotR;
          vx *= -1;
        }
        if (d.y < this.dotR) {
          d.y = this.dotR;
          vy *= -1;
        } else if (d.y > this.h - this.dotR) {
          d.y = this.h - this.dotR;
          vy *= -1;
        }
